...
[`screensaver/`](screensaver/) is a native macOS screen saver of the same flowing-current effect: the bands drift slowly, and two radiance glows pulse and breathe. It's built from pre-rendered gradient layers composited with Core Animation, so it runs on the GPU with next to no CPU cost.

```sh
cd screensaver
.venv/bin/python render_layers.py   # regenerate layers/ (only needed after tweaks)
./install.sh                        # builds ComfyDark.saver and installs it
```

Then pick **ComfyDark** under **System Settings → Screen Saver** (in the *Other* section). Building from source needs Xcode's command line tools (`swiftc`).
